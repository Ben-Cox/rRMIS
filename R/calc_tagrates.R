...
#' @param release_data 
#'
#' @return A tibble with juvenile tag rates for each release in `release_data`
#' 
#'
calc_tagrates <- function(release_data){
  ...
  summarise(Ad_CWT=sum(Ad_CWT,na.rm=TRUE), 
            Ad_NoCWT=sum(Ad_NoCWT,na.rm=TRUE), 
            Unclipped_CWT=sum(Unclipped_CWT,na.rm=TRUE), 
            Unclipped_NoCWT=sum(Unclipped_NoCWT,na.rm=TRUE)) %>% 
  left_join(select(release_data,tag_code_or_release_id,comp_key,rel_id,DIT), by=c("comp_key"="comp_key","rel_id"="rel_id")) %>% 
  mutate(NonMS=Ad_CWT/(Ad_CWT+Ad_NoCWT+Unclipped_CWT+Unclipped_NoCWT), 
         MS=Ad_CWT/(Ad_CWT+Ad_NoCWT)) %>% 
  ungroup() %>% 
